...
export default function DashboardClient({ totalUsers, growth }: DashboardClientProps) {
  const { t } = useLanguage();
  ...
  return (
    <div className="min-h-screen bg-transparent dark:bg-[url('https://grainy-gradients.vercel.app/noise.svg')] dark:bg-blend-multiply text-slate-800 dark:text-slate-200 p-4 md:p-8 lg:p-10 space-y-8 animate-in fade-in duration-1000 relative overflow-hidden transition-colors">
          ...
          <div className="absolute right-10 top-1/2 -translate-y-1/2 hidden lg:flex flex-col gap-4">
            <div className="w-64 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-5 shadow-xl dark:shadow-2xl animate-[float_6s_ease-in-out_infinite] transition-colors">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-slate-500 dark:text-slate-400 font-bold transition-colors">{t("serverLoad")}</p>
            ...
            <div className="w-64 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-5 shadow-xl dark:shadow-2xl animate-[float_6s_ease-in-out_infinite_1s] ml-12 transition-colors">
              <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-slate-500 dark:text-slate-400 font-bold transition-colors">{t("activeUsers")}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
        {[
          { title: t("totalRevenue"), value: "$428,920.00", sub: "+14.5% from last month", icon: DollarSign, trend: "up", gradient: "from-indigo-500 to-purple-500", delay: "delay-100" },
          { title: t("totalUsers"), value: totalUsers.toLocaleString(), sub: `+${growth}% new accounts`, icon: Users, trend: "up", gradient: "from-sky-500 to-blue-600", delay: "delay-200" },
          { title: t("activeOrders"), value: "8,234", sub: "Currently processing", icon: PackageCheck, trend: "neutral", gradient: "from-emerald-500 to-teal-500", delay: "delay-300" },
          { title: t("conversionRate"), value: "4.2%", sub: "-0.4% from yesterday", icon: Target, trend: "down", gradient: "from-rose-500 to-orange-500", delay: "delay-500" },
        ].map((stat, idx) => (
        ...
        <div className="xl:col-span-2 bg-white dark:bg-white/[0.03] backdrop-blur-2xl border border-slate-200 dark:border-white/5 rounded-[40px] shadow-sm overflow-hidden flex flex-col transition-colors">
          <div className="p-8 border-b border-slate-100 dark:border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50 dark:bg-white/[0.01]">
            <div>
              ...
              <p className="text-sm font-medium text-slate-500 mt-1">{t("realTimeStream")}</p>
            </div>
          ...
          <div className="flex-1 overflow-x-auto custom-scrollbar p-6">
            <table className="w-full text-left border-separate border-spacing-y-2">
              <thead>
                <tr>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{t("transactionId")}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{t("userProfile")}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{t("volume")}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{t("statusText")}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-center">{t("action")}</th>
                </tr>
              </thead>
              <tbody>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-gradient-to-br from-indigo-50 dark:from-indigo-900/50 to-purple-50 dark:to-purple-900/50 border border-slate-200 dark:border-white/10 rounded-[40px] p-8 relative overflow-hidden group transition-colors">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between gap-8">
              ...
              <div>
                <p className="text-slate-500 dark:text-white/70 text-sm font-bold uppercase tracking-widest mb-2 transition-colors">{t("networkLiquidity")}</p>
          ...
          <div className="bg-white dark:bg-white/[0.03] backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-[40px] p-8 flex-1 flex flex-col transition-colors">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-lg font-black text-slate-900 dark:text-white transition-colors">{t("systemLogs")}</h3>
            </div>
